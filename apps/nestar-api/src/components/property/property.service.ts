...
import { PropertiesInquiry, PropertyInput, AgentPropertiesInquiry, AllPropertiesInquiry,  OrdinaryInquiry, } from '../../libs/dto/property/property.input';
...
export class PropertyService {
  ...
  public async createProperty(input: PropertyInput): Promise<Property> {
    try {
      ...
      console.log('PropertyService ~ createProperty => err:', err.message);
      if (err.message.includes('E11000')) {
        throw new BadRequestException('This property already exists');
      } else {
...
public async getVisited(memberId: ObjectId, input: OrdinaryInquiry): Promise<Properties> {
  ...
  public async getAllPropertiesByAdmin(input: AllPropertiesInquiry,): Promise<Properties> {
    ...
    const result = await this.propertyModel
      ...
      .exec();
    console.log('PropertyService => result:', result[0].list[0]);
    if (!result.length)
      throw new InternalServerErrorException(Message.NO_DATA_FOUND);
